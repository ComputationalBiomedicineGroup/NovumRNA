...
include {Indices; OptiType; alignment; HLA_extraction; HLA_HD; StringTie; Create_capture_bed; Filtering; Translation; Translation_2; Annotation; Annotation_2; Metadata_MHCI; Metadata_MHCII; Combine; netMHCpan; netMHCIIpan; final_out_1; final_out_2} from "/home/ausserh/projects/2021/CRCnoncanonical/nonCanonicalNeoAG/novumRNA_modules.nf"
...
workflow analysis {
  ...
  Translation(StringTie.out[0].join(Annotation.out[0]).transpose(), params.genome, params.reference_GTF, params.ref_proteome, params.peptide_length)
  ...
  netMHCpan(Filtering.out[1].join(OptiType.out).join(batch_raw_data_ch).transpose())
  netMHCIIpan(Filtering.out[3].join(HLA_HD.out).join(batch_raw_data_ch).transpose())
...
workflow capture_bed_gtf {
  ...
  Create_capture_bed(input_ch_capture_gtf.collect(), params.tpm_max_diff, params.cov_max_diff)
}