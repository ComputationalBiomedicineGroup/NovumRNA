# nf-core/novumrna: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/novumrna/usage](https://nf-co.re/novumrna/usage)
...
```console
nextflow run nf-core/novumrna --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/novumrna
```
...
First, go to the [nf-core/novumrna releases page](https://github.com/nf-core/novumrna/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
