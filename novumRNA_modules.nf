...
process 'Indices' {
  ...
  script:
    ...
    if [ -z $star_index ]; then
      /STAR-2.7.9a/source/STAR --runThreadN ${task.cpus} --runMode genomeGenerate \
      --genomeDir star_index --genomeFastaFiles $genome --limitGenomeGenerateRAM 45000000000
    else
...
process 'OptiType' {
  ...
  reads = (meta.libType == "PE") ? fastq[0] + " " + fastq[1] : fastq
    ...
    if [ $HLA_types_I == "HLA_default.txt" ]; then
       /yara-build/bin/yara_mapper --version-check 0 -e 3 -t ${task.cpus} \
       -f bam $HLA_ref $fastq | samtools view -@ ${task.cpus} -h -F 4 -b1 \
       | samtools sort -o "${meta.ID}_mapped_1_sorted.bam"
...
process 'alignment' {
  ...
  // Check if genomeDir is provided and not empty
    ...
    reads_hisat = (meta.libType == "PE") ? "-1 <(zcat -f " + fastq[0] + ")" + " -2 <(zcat -f " + fastq[1] + ")" : "-U <(zcat -f " + fastq + " )"
    ...
    zcat = (fastq[0].getExtension() == "gz") ? "--readFilesCommand zcat" : ""
    """
    if [ $aligner == "hisat" ]; then
      /hisat2-2.1.0/hisat2 -p ${task.cpus} -x $index_hisat $reads_hisat \
      | samtools sort -O BAM > "${meta.ID}_Aligned.sortedByCoord.out.bam"
      ...
      --readFilesIn $reads_star ${two_pass} --outReadsUnmapped Fastx $zcat \
      --outSAMtype BAM Unsorted --outSAMstrandField intronMotif ${riboseq}
...
process 'StringTie' {
  ...
  output:
      ...
      tuple val (meta), path ("${meta.ID}_vaf_genes.tsv")


  script:
  """
  stringtie -p 1 -o "${meta.ID}_stringtie.gtf" $bam -G $reference_gtf
  python3 /scripts/rename_stringtie.py --stringtie_out "${meta.ID}_stringtie.gtf" \
  --gtf_out "${meta.ID}_stringtie_renamed.gtf" --vaf_out "${meta.ID}_vaf.tsv"

  /bedtools2/bin/bedtools intersect -loj -f 0.9 -s -a "${meta.ID}_vaf.tsv" -b $genes | \
  sed 's/;\\(STRG\\)/;\t\\1/' | sed 's/\\-1\\b/No_gene/g' | \
  awk -v OFS='\t' '!seen[\$4,\$5,\$9]++' > "${meta.ID}_vaf_genes.tsv"
  """
}
...
process 'Filtering' {
  ...
  if [ -s "${meta.ID}_Differential.fasta" ]; then
        ...
        seqkit grep -j ${task.cpus} -n -v -f <(seqkit seq -n "${meta.ID}_Differential.fasta") \
        $peptides_class_I > "${meta.ID}_peptides_I_reduced.fasta"
  else
        # The file is empty.
  ...
  for filtered_file in split_fastas/*.fasta; do
    ...
    seqkit common -j ${task.cpus} -s "${meta.ID}_peptides_I_reduced.fasta" \
    "\$filtered_file" > "${meta.ID}_\${peptide_length}_common_split.fasta"
  done

  cat *_common_split.fasta  > "${meta.ID}_common_I_peptides.fasta"

  seqkit grep -j ${task.cpus} -n -v -f <(seqkit seq -n "${meta.ID}_common_I_peptides.fasta") $peptides_class_I | \
  seqkit rmdup -j ${task.cpus} > "${meta.ID}_peptides_I_filtered.fasta"
...
process 'Create_capture_bed' {
  ...
  /bedtools2/bin/bedtools sort | /bedtools2/bin/bedtools merge -s -c 6 -o distinct -i - \
  | awk -v OFS='\t' '{print \$0, "EXON"}' | awk -v OFS='\t' '{print \$0, "1000"}' \
  | awk -v OFS='\t' '{print \$1,\$2,\$3,\$5,\$6,\$4}' > All_exon.bed

  # Intron
  cat All.gtf | awk 'BEGIN{OFS="\t";} \$3=="transcript" {print \$1,\$4-1,\$5,\$8,\$9,\$7}' \
  | /bedtools2/bin/bedtools sort | /bedtools2/bin/bedtools subtract -s -a stdin -b All_exon.bed | \
  /bedtools2/bin/bedtools sort | /bedtools2/bin/bedtools merge -s -c 6 -o distinct -i - \
  | awk -v OFS='\t' '{print \$0, "INTRON"}' | awk -v OFS='\t' '{print \$0, "1000"}' \
  | awk -v OFS='\t' '{print \$1,\$2,\$3,\$5,\$6,\$4}' > All_intron.bed

  # Intergenic
  grep "chr" All.gtf | grep -w "+" | grep -v "_alt" | grep -v "_random" | grep -v "chrUn" \
  | /bedtools2/bin/bedtools sort -i - | /bedtools2/bin/bedtools complement -i stdin \
  -g $genome_length | awk -v OFS='\t' '{print \$0, "INTERGENIC"}' \
  | awk -v OFS='\t' '{print \$0, "1000"}' | awk -v OFS='\t' '{print \$0, "+"}' > All_intergenic_plus.bed
  
  grep "chr" All.gtf | grep -w "-" | grep -v "_alt" | grep -v "_random" \
  | grep -v "chrUn" | /bedtools2/bin/bedtools sort -i - | \
  /bedtools2/bin/bedtools complement -i stdin -g $genome_length | \
  awk -v OFS='\t' '{print \$0, "INTERGENIC"}' | awk -v OFS='\t' '{print \$0, "1000"}' \
  | awk -v OFS='\t' '{print \$0, "-"}' > All_intergenic_minus.bed
  ...
  cat All_capture_differential_1.bed | /bedtools2/bin/bedtools sort | \
  /bedtools2/bin/bedtools merge -s -c 4,4,6,5,5 -o mean,max,distinct,mean,max \
  -d 50 -i stdin > All_capture_differential_1_merged.bed

  /scripts/capture_bed_differential_2.py --BED All_capture_differential_1_merged.bed \
  --tpm_max_diff $tpm_max_diff --cov_max_diff $cov_max_diff --Out capture_bed_differential.bed
...
process 'Protein_to_peptides' {
  ...
  if [ $ref_pep == "Test_ref_pep.txt" ]; then
    for len in $length_1; do
      /scripts/protein_2_peptide.py --fasta_in $proteins \
      --fasta_out "Control_peptides_len_\${len}.fasta" --pep_len \$len --window_shift 1
...
process 'Translation' {
  ...
  grep "chr" "${meta.ID}_stringtie_short_0.gtf" | grep -v "_alt" \
  | grep -v "_random" | grep -v "chrUn" > "${meta.ID}_stringtie_short.gtf"
  ...
  --closest_out "${meta.ID}_closest_reference.tsv" --matching_out "${meta.ID}_translated.tsv" \
  --Regions_bed "${meta.ID}_overlap_clean.bed" --peptide_length $peptide_length \
  --Bed_out "${meta.ID}_peptide_regions.bed" --Overlaps_out "${meta.ID}_overlaps.tsv" \
  --Regions_fasta "${meta.ID}_regions.fasta" --Peptides_out "${meta.ID}_peptides.fasta" --Ref_range $ref_range
  ...
  split -l\$((`wc -l < "${meta.ID}_peptide_regions.bed"`/$split)) "${meta.ID}_peptide_regions.bed" \
  "${meta.ID}_peptide_regions.split.bed" -da 4
  """
}
...
process 'Capture_regions' {
  ...
  /scripts/Clean_overlapers.py --BED "${meta.ID}_stringtie_specific_new.bed" --GTF $GTF \
  --TPM_min_novel $tpm_min_novel --Cov_min_novel $cov_min_novel \
  --Out_anno "${meta.ID}_annotation.bed" --Out_bed "${meta.ID}_overlap.bed" \
  --TPM_min_diff $tpm_min_diff --Cov_min_diff $cov_min_diff
  """
}
...
process 'BAM_coverage' {
  ...
  samtools view -bq $quality -@ ${task.cpus} -L "${meta.ID}_regions.bed" $bam \
  | samtools rmdup - "${meta.ID}_survivor_coverage.bam"

  java -jar /jvarkit/dist/sortsamrefname.jar \
  --samoutputformat SAM "${meta.ID}_survivor_coverage.bam" > "${meta.ID}_survivor_coverage_refname.sam"
  ...
  java -jar /jvarkit/dist/biostar154220.jar -n 100 --samoutputformat SAM \
  "${meta.ID}_survivor_coverage_refname.bam" > "${meta.ID}_survivor_coverage_limited.sam"
  sed -i '1{/^@/!d}' "${meta.ID}_survivor_coverage_limited.sam"
  samtools view -bh "${meta.ID}_survivor_coverage_limited.sam" \
  | samtools sort > "${meta.ID}_survivor_coverage_limited_sorted.bam" 
  samtools index "${meta.ID}_survivor_coverage_limited_sorted.bam"

  java -jar /jvarkit/dist/sam4weblogo.jar -F tabular -r "${meta.ID}_regions.bed" \
  "${meta.ID}_survivor_coverage_limited_sorted.bam" \
  | grep -v '>' | grep -v "\\--" > "${meta.ID}_${bed.extension}_coverage_reads.tsv"

  sed -i '/locked/d' "${meta.ID}_${bed.extension}_coverage_reads.tsv"
  /scripts/process_subreads.py --tsv_file "${meta.ID}_${bed.extension}_coverage_reads.tsv" \
  --tsv_out "${meta.ID}_${bed.extension}_selected_sequences.bed" --BAM_cov $BAM_cov

  awk '{ printf ">%s\\n%s\\n",\$4"_"\$7,\$5 }' "${meta.ID}_${bed.extension}_selected_sequences.bed" \
  | seqkit sort -n -i > "${meta.ID}_${bed.extension}_selected_sequences.fasta"

  rm *_refname.sam
  rm *_refname.bam
  rm *_limited_sorted.bam
  rm *_coverage_limited.sam
  """
}
...
process 'Translation_2' {
  ...
  script:
  """
  /scripts/translate_with_snps.py --fasta_in $selected --peptides $peptides \
  --regions $regions --tsv_out "${meta.ID}_patient_specific_ncnas.tsv" \
  --tsv_out_meta "${meta.ID}_patient_specific_ncnas_meta.tsv"
  """
}
...
process 'pVACbind_class_I' {
  ...
  script:
    ...
    # Use OptiType output as input
      Class_I=\$(tr -s '\t'  '\n'<$optiTypeOutput | sed -n 11,16p \
      | awk '\$1="HLA-"\$1' | awk '{print \$1}' | paste -s -d, -)
    else
    ...
    for filtered_file in split_fastas/*.fasta; do
      ...
      pvacbind run --iedb-install-directory /opt/iedb \$filtered_file \
      "${meta.ID}_\$peptide_length" \$Class_I -e1 \$peptide_length "NetMHCpan" \
      "${meta.ID}_\$peptide_length"
    done

    awk 'FNR==1 && NR!=1 {next} {print}' */MHC_Class_I/*all_epitopes.tsv \
    > "${peptides_I.baseName}_peptides_I_binding.tsv"
    """
}
...
process 'pVACbind_class_II' {
  ...
  script:
    ...
    for filtered_file in split_fastas/*.fasta; do
      ...
      mkdir "${meta.ID}_\$peptide_length"
      pvacbind run --iedb-install-directory /opt/iedb \$filtered_file \
      "${meta.ID}_\$peptide_length" \$Class_II -e1 \$peptide_length "NetMHCIIpan" \
      "${meta.ID}_\$peptide_length"
    done

    awk 'FNR==1 && NR!=1 {next} {print}' */MHC_Class_II/*all_epitopes.tsv > \
    "${peptides_II.baseName}_peptides_II_binding.tsv"
    """
}
...
process 'Final_MHCI' {
  ...
  --BED_old "${meta.ID}_final_out_combined_0.tsv" --Out "${meta.ID}_final_class_I_prediction.tsv"
    """
}
...
process 'Rerun_samplesheet' {
  ...
  read2 = reads[1]
    ...
    if [ $hlahd == "test_final.result.txt" ]; then
      ...
      echo "$meta.ID,$read1,$read2,${outdir}intermediate_results/05_StringTie/$gtf,\\
    ${outdir}intermediate_results/05_StringTie/$vaf,${outdir}intermediate_results/02_alignment/$bam,\\
    ${outdir}intermediate_results/02_alignment/$bai,${outdir}intermediate_results/01_OptiType/$opti,\\
    ${outdir}intermediate_results/04_HLA_HD/HLA_HD_out/test/result/$hlahd,$hla_I,$hla_II" \\
      >> "${meta.ID}_rerun_samplesheet.csv"
    ...
    ${outdir}intermediate_results/05_StringTie/$vaf,${outdir}intermediate_results/02_alignment/$bam,\\
    ${outdir}intermediate_results/02_alignment/$bai,${outdir}intermediate_results/01_OptiType/$opti,\\
    ${outdir}intermediate_results/04_HLA_HD/HLA_HD_out/$meta.ID/result/$hlahd,$hla_I,$hla_II" \\
      >> "${meta.ID}_rerun_samplesheet.csv"
      sed -i 's/null//g' "${meta.ID}_rerun_samplesheet.csv"
    fi  
    """
